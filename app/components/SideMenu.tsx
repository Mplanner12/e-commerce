...
const SideMenu: React.FC<SideMenuProps> = ({ navOpen }) => {
  ...
  return (
        <aside className={`${navOpen ? 'left-0' : 'left-[-140%]'} w-full lg:w-1/4 bg-gray-800 text-white p-4 absolute lg:left-0 transition-all top-0 bottom-0 lg:relative z-10`}>
            ...
            <nav className='h-[80vh] flex flex-col justify-between'>
                <div>
                    {sidebarItems.map((item, index) => (
                        <a key={index} href={item.link} className="flex items-center p-2 hover:bg-gray-700">
